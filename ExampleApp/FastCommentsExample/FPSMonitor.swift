    ...
    final class FPSMonitor: ObservableObject {
        ...
        func start() {
            ...
            let link = CADisplayLink(
                target: DisplayLinkTarget { [weak self] link in
                    self?.tick(link)
        ...
        func stop() {
            ...
            if !fpsSamples.isEmpty {
                ...
                print(
                    "[Benchmark FPS] Monitor stopped — avg: \(String(format: "%.1f", avg)), min: \(String(format: "%.1f", minFPS == .infinity ? 0 : minFPS)), max: \(String(format: "%.1f", maxFPS)), samples: \(fpsSamples.count), dropped: \(droppedFrameCount)"
                )
            }
        }
        ...
        private func tick(_ link: CADisplayLink) {
            ...
            if elapsed >= 1.0 {
                ...
                print(
                    "[Benchmark FPS] \(String(format: "%.1f", fps)) fps (min: \(String(format: "%.1f", minFPS)), max: \(String(format: "%.1f", maxFPS)))"
                )
