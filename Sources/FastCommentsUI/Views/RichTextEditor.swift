    ...
    public final class RichTextEditorContext: ObservableObject {
        ...
        func toggleStrikethrough() {
            ...
            if range.length > 0 {
                ...
                let hasStrike =
                    (mutable.attribute(.strikethroughStyle, at: range.location, effectiveRange: nil) as? Int)
            ...
            if range.length > 0 {
                ...
                if isCode {
                    ...
                    let codeFont = UIFont.monospacedSystemFont(
                        ofSize: UIFont.preferredFont(forTextStyle: .subheadline).pointSize, weight: .regular)
        ...
        func toggleCodeBlock() {
            ...
            if range.length > 0 {
                let mutable = NSMutableAttributedString(attributedString: textView.attributedText)
                let isCodeBlock =
                    mutable.attribute(.codeBlock, at: range.location, effectiveRange: nil) as? Bool ?? false
        ...
        func applyLink(url: URL, label: String) {
            ...
            if range.length > 0 {
                ...
                let linkText = NSAttributedString(
                    string: label.isEmpty ? url.absoluteString : label,
                    attributes: [
                        .link: url,
        ...
        private func toggleTrait(_ trait: UIFontDescriptor.SymbolicTraits, on textView: UITextView) {
            ...
            if range.length > 0 {
                let mutable = NSMutableAttributedString(attributedString: textView.attributedText)
                let currentFont =
                    mutable.attribute(.font, at: range.location, effectiveRange: nil) as? UIFont
        ...
        class Coordinator: NSObject, UITextViewDelegate {
            ...
            func textView(
                ...
                textView.attributedText.enumerateAttribute(.mentionUserId, in: fullRange) { value, attrRange, stop in
                    ...
                    if NSIntersectionRange(attrRange, range).length > 0
                        || range.location == attrRange.location + attrRange.length
                    {
